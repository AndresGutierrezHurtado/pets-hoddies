...
app.post('/addnewproduct', (req, res) => {
  const sql = 'INSERT INTO productos (nombre, descripcion, precio, img, stock) VALUES (?, ?, ?, ?)'

  db.query(sql, ['Nuevo producto', 'Nuevo producto', 10000, '/img/productos/nf.jpg', 1], (err) => {
    if (err) {
...
app.post('/profile/product/update', (req, res) => {
  ...
  const updateUserQuery = 'UPDATE productos SET nombre = ?, descripcion = ?, precio = ?, stock = ? WHERE producto_id = ?';
      ...
      if (result.affectedRows > 0) {
        ...
        db.query(getProductQuery, [id], (err, productResult) => {
          if (err) {
            ...
            return res.status(200).json({ message: 'Perfil de producto actualizado correctamente', product: updatedProduct });
          }
        });
      } else {
...
app.post('/upload/image/product', upload.single('image'), (req, res) => {
  ...
  db.query(updateQuery, [pathProductImg, id], (err, result) => {
    if (err) {
      ...
      if (result.affectedRows > 0) {
        return res.status(200).json({ message: 'la foto de producto actualizado correctamente'});
      } else {
  ...
  db.query(userQuery, [email], async (err, result) => {
    if (err) {
      ...
      if (result.length > 0) {
        ...
        const insertUserQuery = 'INSERT INTO usuarios (nombre, email, usuario, contrasena, rol_id) VALUES (?, ?, ?, ?, ?)';
        db.query(insertUserQuery, [fullName, email, firstName, facebookId, 1], (err, result) => {
          if (err) {
            console.error('Error al insertar usuario de Facebook en la base de datos:', err);
            res.status(500).json({ error: 'Error al registrar usuario de Facebook' });
          } else {
  ...
  db.query(existingUserQuery, [email], async (err, result) => {
    if (err) {
      ...
      if (result.length === 0) {
        ...
        const insertUserQuery = 'INSERT INTO usuarios (nombre, usuario, email, contrasena, telefono, rol_id) VALUES (?, ?, ?, ?, ?, ?)';
        db.query(insertUserQuery, [nombre, username, email, hashedPassword, telefono, rol_id], (err) => {
          if (err) {
...
app.post('/profile/user/update', async (req, res) => {
  ...
  Jwt.verify(token, secretKey, (err, decoded) => {
    if (err) {
      ...
      const updateUserQuery = 'UPDATE usuarios SET nombre = ?, usuario = ?, email = ?, telefono = ?, address = ? WHERE usuario_id = ?';

      if (tokenId != id && decoded.rol !=2) {
        return res.status(403).json({ error: 'No tienes permiso para editar este perfil' });
      }

      db.query(updateUserQuery, [fullname, username, email, phonenumber, address, id], (err, result) => {
        if (err) {
          ...
          if (result.affectedRows > 0) {
            if (tokenId == id) {
              ...
              const newToken = Jwt.sign(tokenData, secretKey, { expiresIn: '5h' });
              return res.status(200).json({ message: 'Perfil de usuario actualizado correctamente', token: newToken });              
            } else {
